// HTTP client for the Auravox REST API.
//
...
export interface AuravoxFetchInit {
    method?: string;
...
export interface AuravoxFetchResponse {
    ok: boolean;
    status: number;
    statusText: string;
...
export type AuravoxFetch = (
    url: string,
    init?: AuravoxFetchInit,
) => Promise<AuravoxFetchResponse>;
...
export interface AuravoxClientOptions {
    baseUrl?: string;
    apiKey?: string;
...
export class AuravoxClient extends _GeneratedClient implements SpecProvider {
    readonly baseUrl: string;
    readonly apiKey: string | undefined;
    private readonly fetchImpl: AuravoxFetch;
    private readonly timeoutMs: number;
    private readonly headers: Record<string, string>;
    private readonly specCache = new Map<string, NodeSpec>();
    private specVersionCache: string | null = null;

    constructor(opts: AuravoxClientOptions = {}) {
        super();
        const rawBase =
            opts.baseUrl ??
            getRuntimeEnv("AURAVOX_API_URL") ??
            "http://localhost:8000";
        this.baseUrl = rawBase.replace(/\/+$/, "");
        this.apiKey = opts.apiKey ?? getRuntimeEnv("AURAVOX_API_KEY");
        this.fetchImpl = opts.fetch ?? (globalThis.fetch as unknown as AuravoxFetch);
