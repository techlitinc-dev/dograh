...
`AuravoxClient` mixes in this class to get HTTP methods for every route
...
from auravox_sdk._generated_models import (
{imports}
)


class _GeneratedClient:
    # `AuravoxClient.__init__` installs `self._request` (see client.py).
...
// Regenerate with `./scripts/generate_sdk.sh`.
//
// `AuravoxClient` extends this base to get HTTP methods for every route
