...
The generated files live under `auravox_sdk.typed` and are committed to
the repository so `pip install auravox-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m auravox_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/auravox_sdk/typed

    python -m auravox_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from auravox_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from auravox_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from auravox_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m auravox_sdk.codegen",
        description="Generate typed SDK dataclasses from the Auravox node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="Auravox backend base URL")
