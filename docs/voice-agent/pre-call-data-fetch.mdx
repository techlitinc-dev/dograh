...
2. Auravox sends a **POST** request to your configured endpoint with a standardized payload.
...
| **Endpoint URL** | The URL Auravox will send the POST request to. |
...
Auravox sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/auravox/pre-call", async (req, res) => {
