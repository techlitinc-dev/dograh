...
description: Add your Auravox agent to any website so visitors can talk to it by voice or chat with it by text.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="auravox-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio/chat pipeline plus a JavaScript API on `window.AuravoxWidget`.                | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `auravox-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.AuravoxWidget`. Code that registers callbacks must wait for the widget to be available.
...
```js
window.AuravoxWidget.setContext({
...
```html
<!-- Paste the auravox embed snippet from the dashboard somewhere on the page -->
<div id="auravox-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.AuravoxWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('auravox-inline-container');
      if (window.AuravoxWidget && container) {
        const { isInitialized } = window.AuravoxWidget.getState();
        if (isInitialized) window.AuravoxWidget.refresh();
        else window.AuravoxWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="auravox-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.AuravoxWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.AuravoxWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.AuravoxWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.AuravoxWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.AuravoxWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.AuravoxWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.AuravoxWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.AuravoxWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |
| `window.AuravoxWidget.setContext(vars)`         | Merge visitor context for the next call — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless.         |
...
| Method / Callback                              | Description                                                                                                                                                  |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.AuravoxWidget.startChat()`              | Start a conversation. The agent's greeting arrives via `onMessage`.                                                                                          |
| `window.AuravoxWidget.sendMessage(text)`        | Send a visitor message. Returns a Promise that resolves with the updated transcript (array of turns), or `null` if the message couldn't be delivered.        |
| `window.AuravoxWidget.getMessages()`            | Current transcript as an array of turns: `{ id, status, user_message, assistant_message }`, each message being `{ text, created_at }`.                       |
| `window.AuravoxWidget.onMessage(cb)`            | Fires once per new agent reply. Callback receives `(text, turn)`.                                                                                            |
| `window.AuravoxWidget.onChatStateChange(cb)`    | Fires on every chat state change. States: `idle`, `starting`, `ready`, `waiting` (agent is replying), `ended`, `expired`, `error`.                           |
| `window.AuravoxWidget.onError(cb)`              | Fires on errors. Callback receives an `Error` object.                                                                                                        |
| `window.AuravoxWidget.setContext(vars)`         | Merge visitor context for the next conversation — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless. |
...
<script>
  window.addEventListener('load', () => {
    window.AuravoxWidget.onMessage((text) => {
    ...
    document.getElementById('open-chat').addEventListener('click', () => {
      window.AuravoxWidget.startChat();
    });
    ...
    document.getElementById('send-btn').addEventListener('click', async () => {
      ...
      document.getElementById('transcript').appendChild(p);
      await window.AuravoxWidget.sendMessage(input.value);
      input.value = '';
    });
  });
</script>
...
<Note>
**About timing.** The widget script loads asynchronously, so `window.AuravoxWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.AuravoxWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.AuravoxWidget.onError((err) => {
    console.error('Auravox error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.AuravoxWidget.onStatusChange((s) => setStatus(s));
    window.AuravoxWidget.onError((err) => console.error('Auravox error:', err.message));
  }, []);
...
```js
window.AuravoxWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.AuravoxWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
