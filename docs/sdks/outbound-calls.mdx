...
description: "Trigger a Auravox voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the Auravox UI.

## Prerequisites

- A Auravox [API key](/configurations/api-keys) exported as `AURAVOX_API_KEY`
...
with AuravoxClient(api_key="YOUR_API_KEY") as client:
...
const client = new AuravoxClient({ apiKey: "YOUR_API_KEY" });
