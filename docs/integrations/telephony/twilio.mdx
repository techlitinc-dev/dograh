...
description: "Configure Twilio for voice communication in Auravox AI"
---

## Overview

Twilio is a cloud communications platform that enables voice calling, messaging, and video capabilities. Auravox AI's Twilio integration provides seamless connectivity for your voice agents.
...
Inbound routing is driven by the phone number itself — there is a **single webhook URL** for the entire org, and Auravox resolves which agent to run from the called number's assigned inbound workflow. **When you save an inbound workflow on a phone number, Auravox automatically pushes the webhook URL to that number's `VoiceUrl` in your Twilio account** (provided the credentials are correct and the number belongs to that account).
...
3. In the **Voice Configuration** section, confirm:
   - **Webhook** is set to: `https://api.auravox.ai/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`

   <Note>
     Auravox pushed this URL automatically when you saved the inbound workflow
     in Step 1. If the field is empty, shows a different URL, or Auravox
     ...
     `POST`, and click **Save Configuration**. On self-hosted Auravox,
...
3. Check call logs in both Auravox AI dashboard and Twilio Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls go to voicemail">
    ...
    exists in your Auravox telephony configuration and has an **Inbound
    workflow** assigned - Verify Auravox AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    (Auth Token in Auravox matches Twilio Console) - Verify WebSocket connection
