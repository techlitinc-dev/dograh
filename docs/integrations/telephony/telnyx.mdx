...
description: "Configure Telnyx for voice communication in Auravox AI"
---

## Overview

Telnyx is a cloud communications platform that provides programmable voice via its Call Control API. Auravox AI's Telnyx integration uses Call Control plus WebSocket media streaming to power your voice agents.
...
- A **Call Control Application** (optional — leave the field blank in Auravox and we'll auto-create one for you on save, with the inbound webhook URL pre-set)
...
3. (Optional) Navigate to **Call Control** → **Applications** and create (or open) the application you'll use with Auravox, then copy its **Connection ID** (Call Control App ID). Skip this if you want Auravox to auto-create the Call Control Application for you on save.
4. Navigate to **Numbers** → **My Numbers** and assign your phone numbers to the Call Control Application you'll use with Auravox (if you're letting Auravox auto-create the application, do this after saving the configuration in Step 2)
...
3. Enter your credentials:
   - API Key
   - Call Control App ID (Connection ID) — *optional*. Leave blank and Auravox will auto-create a Call Control Application on save (with the inbound webhook URL already configured) and store its Connection ID on this configuration.
...
5. Open the configuration you just created and add at least one **phone number** (with country code, in [E.164](https://en.wikipedia.org/wiki/E.164) format — `+` then country code then number, no spaces or dashes, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If Auravox auto-created the Call Control Application for you, you still
     ...
     is named `auravox-<random>` — its Connection ID is shown on the saved
     configuration.
   </Note>
...
Telnyx delivers inbound webhooks at the **Call Control Application** level — the webhook URL is configured once on the application, and applies to every number assigned to it. **When you save an inbound workflow on a phone number, Auravox automatically pushes the webhook URL to your Call Control Application's `webhook_event_url`** (provided the credentials are correct). If Auravox auto-created the application during configuration save, the webhook URL is already set and this step is a no-op.
...
2. Open the application whose Connection ID you configured in Auravox
3. In **Webhook Settings**, confirm:
   - **Webhook URL** is set to: `https://api.auravox.ai/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`
4. Make sure the phone numbers you want to use for inbound are assigned to this application
   ...
   <Note>
     ...
     Control Application — Auravox matches the inbound call to the right
     agent using the called number's inbound workflow assignment. If the
     field is empty, shows a different URL, or Auravox surfaced a sync
     warning on save, the auto-push failed — most often because the API
     Key or Connection ID in Auravox is incorrect. Paste the URL into the
     field yourself, set the method to `POST`, and save. On self-hosted
     Auravox, replace `api.auravox.ai` with your backend domain.
   </Note>
...
3. Check call logs in both Auravox AI dashboard and Telnyx Portal
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls aren't answered">
    ...
    `https://api.auravox.ai/api/v1/telephony/inbound/run` - Ensure the webhook
    ...
    Auravox AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    your Auravox telephony configuration - Verify WebSocket connection
...
- Use a dedicated Call Control Application for Auravox so the shared webhook URL doesn't conflict with other systems
