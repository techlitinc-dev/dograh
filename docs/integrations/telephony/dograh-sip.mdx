---
title: "Auravox Managed SIP"
description: "Connect your own SIP carrier or PBX to the SIP domain Auravox provisions for your organization"
---

## What you get, and what you bring

Every Auravox organization is provisioned with a telephony configuration named
**Auravox Cloudonix SIP**. It gives you a SIP domain that is already connected
to the Auravox voice pipeline — you do not need a Cloudonix account, and there
is nothing to pay for or set up on the Cloudonix side.

What it does **not** include is phone service. Auravox does not sell or resell
phone numbers, and no calls flow until you connect a carrier:

| Auravox provides                                     | You provide                                    |
| --------------------------------------------------- | ---------------------------------------------- |
...
<Note>
  Until you finish the steps below, **Auravox Cloudonix SIP** cannot place a
...
## Step 1: Read your SIP endpoints from Auravox

1. Go to **Telephony configurations** and open **Auravox Cloudonix SIP**
...
4. Note the two values you will need on the carrier side:
   - **Inbound hostname** and **ports** — where your carrier sends calls *to* Auravox
   - **Outbound origin IP address** — the address Auravox's calls arrive *from*

## Step 2: Point your carrier at Auravox (inbound)
...
Auravox needs to know where to send calls when your agent dials out.

1. On your carrier, allow the **origin IP address** from Step 1 to send SIP
   traffic to your trunk. Calls from Auravox are unauthenticated and identified
   by source IP, so this step is required
2. Back in Auravox, under **Outbound trunks**, choose **Add trunk** and fill in:
   - **Name** — any name you choose. Letters, digits and hyphens only
   - **SIP domain** — the hostname of your carrier or PBX, for example
     `sip.example.com`. Auravox puts this in both the SIP `To` header and the
     Request-URI
   - **Region** — the edge Auravox dials your carrier from. The dialog shows
...
rejected by the carrier even though Auravox accepts it — which is why the trunk
assignment matters as soon as you have more than one carrier. With a single
trunk, Auravox uses it for every number and the field can be left alone.
...
Auravox pushes the inbound webhook URL to the Voice Application on the
...
2. Choose **Phone Call**
3. Select **Auravox Cloudonix SIP**, pick your caller ID, enter a destination
