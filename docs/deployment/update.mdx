---
title: "Update"
description: "Update your self-hosted Auravox stack to a newer image version"
---

This guide covers updating a Auravox stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `auravox/` directory if you used `setup_remote.sh`).
...
Auravox publishes two images — `auravox-api` and `auravox-ui` — to both container registries:

- **GitHub Container Registry** — [github.com/orgs/auravox-hq/packages](https://github.com/orgs/auravox-hq/packages)
- **Docker Hub** — [hub.docker.com/u/auravoxai](https://hub.docker.com/u/auravoxai)
...
|-------|-----------|---------|-------------|
| GitHub release tag | `auravox-vX.Y.Z` | `auravox-v1.28.0` | Use when copying the version from a GitHub release |
...
<Warning>
Always update **`auravox-api`** and **`auravox-ui`** to the **same tag**. The two images are built from the same commit and the UI expects API responses in a matching shape — mixing versions will break the app. `update_remote.sh` handles this for you automatically.
</Warning>
...
- Synchronizes the canonical remote keys in `.env` and validates the runtime config that `auravox-init` will render from it.
...
```bash
cd auravox
curl -o update_remote.sh https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/update_remote.sh
bash update_remote.sh
```

You'll be prompted for the target version, defaulting to the most recent release. Accepted forms: bare semver (`1.28.0`), v-prefixed (`v1.28.0`), the full GitHub tag (`auravox-v1.28.0`), or `main` to refresh deployment files from the default branch — the script normalizes them. Non-interactive callers can set it via environment variable and skip the confirmation prompt:

```bash
TARGET_VERSION=1.28.0 AURAVOX_UPDATE_YES=1 bash update_remote.sh
```
...
<CodeGroup>
```bash macOS/Linux
curl -o docker-compose.yaml https://raw.githubusercontent.com/auravox-hq/auravox/main/docker-compose.yaml
curl -o start_docker.sh https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/start_docker.sh && chmod +x start_docker.sh
docker compose down
./start_docker.sh
```
```powershell Windows
Invoke-WebRequest -OutFile docker-compose.yaml https://raw.githubusercontent.com/auravox-hq/auravox/main/docker-compose.yaml; Invoke-WebRequest -OutFile start_docker.ps1 https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/start_docker.ps1; docker compose down; .\start_docker.ps1
```
</CodeGroup>
...
```bash
cd auravox
for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_auravox_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/auravox-hq/auravox/issues) before rolling back.
</Warning>
...
# Track latest main:
git pull
# Or pin to a specific release (git tag format is auravox-vX.Y.Z):
git checkout auravox-v1.28.0
