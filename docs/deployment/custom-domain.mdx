---
title: "Custom Domain"
description: "Deploy Auravox AI with custom domain names and SSL certificates"
---

Deploy Auravox AI with your own custom domain name for a professional production setup. By now, you should be able to create and test a voice agent by following the previous guide to setup the platform on a remote server using [Docker](docker#option-2%3A-remote-server-deployment)
...
Custom domain deployment allows you to run Auravox AI with a personalized domain name (like `voice.yourcompany.com`) instead of using IP addresses. This setup includes:
...
- Auravox AI already running on your server via the [remote deployment](docker#option-2%3A-remote-server-deployment) guide
...
<Note>
You must be at the same place where you had run `setup_remote.sh` from. The directory should contain `auravox/` with the artifacts that got created when `setup_remote.sh` was run.
</Note>
<Note>
You must not move the `auravox/` directory to a different location after running `setup_custom_domain.sh`, since we set up auto certificate renewal script as certbot renewal hook pointing to the `auravox/` directory.
</Note>

```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
- Restart Auravox services through the validated startup wrapper
...
Update `.env` so the canonical remote settings use your domain — `auravox-init` reads these to render nginx's `server_name`. Replace `voice.yourcompany.com` with your actual domain throughout:
...
```bash
sudo nano /etc/letsencrypt/renewal-hooks/deploy/auravox-reload.sh
```
...
```bash
#!/bin/bash
# Copy renewed certificates to auravox certs directory
cp /etc/letsencrypt/live/voice.yourcompany.com/fullchain.pem /home/ubuntu/auravox/certs/local.crt
cp /etc/letsencrypt/live/voice.yourcompany.com/privkey.pem /home/ubuntu/auravox/certs/local.key
chmod 644 /home/ubuntu/auravox/certs/local.crt /home/ubuntu/auravox/certs/local.key

# Restart nginx to load new certificates
cd /home/ubuntu/auravox
...
```bash
sudo chmod +x /etc/letsencrypt/renewal-hooks/deploy/auravox-reload.sh
```
...
1. Verify the certificates were copied correctly: `ls -la auravox/certs/`
2. Run `./remote_up.sh --preflight-only` in `auravox/` to verify the `auravox-init` runtime render matches `.env`
