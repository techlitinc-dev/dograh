...
description: "Configure the speech-to-speech, Auravox-managed, or bring-your-own-key models your Auravox agents use."
---
...
To configure models, open **Models** in your Auravox dashboard:

- **Hosted:** `https://app.auravox.ai/model-configurations`
...
| **Auravox** | Use Auravox-managed LLM, voice, and transcriber models behind one Auravox Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but Auravox uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## Auravox

Use **Auravox** when you want Auravox to manage the model providers for you. This path uses one Auravox Service Key for Auravox-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![Auravox model configuration](../images/model-configuration-auravox.png)

Configure:

| Field | What it controls |
|-------|------------------|
| **Voice** | The Auravox-managed voice to use. |
| **Speed** | The voice playback speed. |
| **Language** | The language behavior, including multilingual auto-detect when available. |
| **API Key** | Your Auravox Service Key. Create Service Keys from **Developers**. |
