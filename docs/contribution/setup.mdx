---
title: Setup
description: Get a Auravox dev environment running and learn the daily commands.
---

Auravox is a Next.js UI (`ui/`), a FastAPI backend (`api/`), and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based voice pipeline (`pipecat/`, a git submodule), backed by Postgres, Redis, and MinIO.

<Tip>
**Using Claude Code or Codex?** Install the [Auravox setup skill](https://github.com/auravox-hq/auravox-plugins) (`/plugin marketplace add auravox-hq/auravox-plugins`, then `/plugin install auravox@auravox`) and ask your agent to *"set up Auravox for development"* — it runs the steps below for you and verifies the stack is healthy.
</Tip>
...
1. Fork [auravox-hq/auravox](https://github.com/auravox-hq/auravox) and clone **your fork**:

```bash
git clone https://github.com/<YOUR_HANDLE>/auravox
cd auravox
```
...
If these steps do not work for you, please open an issue on [GitHub](https://github.com/auravox-hq/auravox/issues).

<AccordionGroup>
<Accordion title="Cloned auravox-hq/auravox directly instead of your fork?">
Run `bash scripts/setup_fork.sh` once. It prompts for your fork URL, points `origin` at it, and adds `upstream` as `auravox-hq/auravox`. See [Fork and upstream remotes](/contribution/reference#fork-and-upstream-remotes).
</Accordion>
...
| `sdk/` | Python/TypeScript SDKs for driving Auravox programmatically |
...
2. Push to your fork (`origin`) and open a pull request against `auravox-hq/auravox:main`.
3. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://github.com/auravox-hq/auravox/issues) and [Ideas](https://github.com/orgs/auravox-hq/discussions/categories/ideas). Issues tagged `good first issue` are a good place to start. For questions while you're working, use the [Auravox Community Slack](https://join.slack.com/t/auravox-community/shared_invite/zt-3zjb5vwvl-j7hRz3_F1SOn5cH~jm5f5g).
