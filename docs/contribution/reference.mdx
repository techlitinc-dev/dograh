...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `auravox-hq/auravox`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, please open a [GitHub issue](https://github.com/auravox-hq/auravox/issues).
...
- `upstream`: `auravox-hq/auravox`, where new commits land

If you cloned `auravox-hq/auravox` directly instead of your fork, run it once (inside the devcontainer is fine):
...
```bash
origin    https://github.com/<YOUR_HANDLE>/auravox.git (fetch/push)
upstream  https://github.com/auravox-hq/auravox.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `auravox-hq/auravox:main`. Never push directly to `upstream`.
</Note>
