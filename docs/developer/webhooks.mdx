...
description: "Context variables available in webhook nodes and the data Auravox sends after a call"
---

Auravox executes **webhook nodes** asynchronously after a workflow run completes. You configure the target URL, HTTP method, headers, and payload template directly in the workflow definition.
...
2. Auravox executes any `webhook` nodes in the workflow asynchronously
...
<Note>
Auravox always includes a top-level `call_disposition` field in the delivered payload. If your template doesn't set one, it is added automatically from `gathered_context.call_disposition`; if your template sets it explicitly, your value is kept.
</Note>
...
@app.post("/webhook/auravox")
async def handle_auravox_webhook(request: Request):
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "http_method": "POST",
    "endpoint_url": "https://your-service.com/webhook/auravox",
    "payload_template": {
      "run_id": "{{workflow_run_id}}",
      "customer": "{{initial_context.customer_name}}",
      "outcome": "{{gathered_context.resolution}}"
