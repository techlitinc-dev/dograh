...
description: "Complete reference for all environment variables used by the Auravox backend"
---
...
Auravox supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get Auravox running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through Auravox Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
| `MPS_API_URL` | `https://services.auravox.ai` | Auravox Managed Platform Services URL |
| `AURAVOX_MPS_SECRET_KEY` | `null` | **Required for non-OSS deployments.** Secret key for authenticating with MPS |
...
Auravox uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
...
<Note>
Presigned URLs point at `S3_ENDPOINT_URL`, so that host must be reachable from the browser. Because browsers fetch transcripts cross-origin, the bucket also needs a CORS rule allowing your app's origin for `GET`/`HEAD` — configure this on the storage server (e.g. via `PutBucketCors`), not in Auravox.
</Note>
...
Carriers dial the media WebSocket back at `/api/v1/telephony/ws/{workflow_id}/{organization_id}/{workflow_run_id}`. Those ids travel in a caller-visible URL, so on their own they are a guessable capability. Setting a secret makes Auravox sign that URL with an HMAC and verify the signature when the socket opens.
...
Controls concurrency for [Campaigns](/core-concepts/campaigns), Auravox's bulk outbound calling feature.
