...
sync_postgres_password() {
    ...
    printf '%s\n' "ALTER USER postgres WITH PASSWORD :'auravox_password';" \
        | docker compose exec -T postgres psql \
        -U postgres \
        -d postgres \
        -v ON_ERROR_STOP=1 \
        -v "auravox_password=$postgres_password" >/dev/null
...
if [[ ! -t 0 ]]; then
    echo "Run the command above from an interactive shell to start Auravox."
    exit 0
fi

read -r -p "Start Auravox now? [Y/n]: " answer
case "$answer" in
    [Nn]*)
        echo "Auravox was not started."
        exit 0
        ;;
esac
