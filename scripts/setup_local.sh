...
REGISTRY="${REGISTRY:-ghcr.io/auravox-hq}"
...
# Download compose file (skip when AURAVOX_SKIP_DOWNLOAD=1 — e.g. local repo testing).
TOTAL_STEPS=2
...
if [[ "${AURAVOX_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml https://raw.githubusercontent.com/auravox-hq/auravox/main/docker-compose.yaml
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        auravox_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_auravox_init.sh ]] || auravox_fail "scripts/run_auravox_init.sh not found. Re-run setup_local.sh without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || auravox_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || auravox_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start Auravox with TURN, run:${NC}"
    echo ""
