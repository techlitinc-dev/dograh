...
# Requires:
#   - `python` in the `auravox` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t auravox-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t auravox-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m auravox_sdk.codegen \
    --input "$SPECS_JSON" \
...
datamodel-codegen \
    ...
    --output "sdk/python/src/auravox_sdk/_generated_models.py" \
...
python -m sdk.codegen.client_codegen \
    --input "$OPENAPI_JSON" \
    --py-out "sdk/python/src/auravox_sdk/_generated_client.py" \
