...
NGINX_UPSTREAM_TEMPLATE="$BASE_DIR/nginx/auravox_upstream.conf.template"
NGINX_UPSTREAM_CONF="/etc/nginx/conf.d/auravox_upstream.conf"

HEALTH_CHECK_ENDPOINT="/api/v1/health"
ACTIVE_CALLS_ENDPOINT="/api/v1/health/active-calls"
AURAVOX_DEVOPS_SECRET_HEADER="X-Auravox-Devops-Secret"
...
if [[ -z "${AURAVOX_DEVOPS_SECRET:-}" ]]; then
  log_error "AURAVOX_DEVOPS_SECRET is not set. Add it to $ENV_FILE before running rolling_update.sh."
  exit 1
fi
if [[ "$AURAVOX_DEVOPS_SECRET" == "change-me-auravox-devops-secret" ]]; then
  log_error "AURAVOX_DEVOPS_SECRET still has the example placeholder value. Replace it in $ENV_FILE."
  exit 1
fi
...
count_active_calls_on_port() {
  ...
  response=$(curl -sS --max-time 3 \
    -H "${AURAVOX_DEVOPS_SECRET_HEADER}: ${AURAVOX_DEVOPS_SECRET}" \
    -w $'\n%{http_code}' \
  ...
  if [[ "$http_code" != "200" ]]; then
    log_error "uvicorn_${port} active-calls endpoint returned HTTP ${http_code}. Check AURAVOX_DEVOPS_SECRET in $ENV_FILE."
    return 1
  fi
