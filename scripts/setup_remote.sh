...
if [[ ! -f "$LIB_PATH" ]]; then
    ...
    curl -fsSL -o "$BOOTSTRAP_LIB" "https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/lib/setup_common.sh"
...
cleanup() {
    ...
    if [[ -n "${SUDO_UID:-}" && -n "${SUDO_GID:-}" && -n "${AURAVOX_DEPLOY_PROJECT_DIR:-}" && -d "$AURAVOX_DEPLOY_PROJECT_DIR" ]]; then
        echo -e "${BLUE}Restoring ownership of $AURAVOX_DEPLOY_PROJECT_DIR to ${SUDO_USER:-uid $SUDO_UID}...${NC}"
        chown -R "$SUDO_UID:$SUDO_GID" "$AURAVOX_DEPLOY_PROJECT_DIR" || true
    fi
}
trap cleanup EXIT
...
if [[ $EUID -ne 0 ]]; then
    auravox_fail "setup_remote.sh must be run as root.\nRe-run with sudo:\n  sudo ./setup_remote.sh"
fi
...
if [[ -z "$SERVER_IP" ]]; then
    auravox_fail "IP address cannot be empty"
fi

if ! auravox_is_ipv4 "$SERVER_IP"; then
    auravox_fail "Invalid IP address format"
fi
...
if [[ "$CERT_MODE" == "auto" ]]; then
    if auravox_is_local_ipv4 "$SERVER_IP"; then
        CERT_MODE="self-signed"
        auravox_warn "$SERVER_IP is a private IP — using a self-signed certificate."
        auravox_warn "For a trusted cert, deploy on a public IP or a domain you own"
        auravox_warn "(https://docs.auravox.ai/deployment/custom-domain)."
    elif ! command -v docker >/dev/null 2>&1; then
        CERT_MODE="self-signed"
        auravox_warn "Docker not found — skipping automatic Let's Encrypt setup and using a self-signed cert."
    else
        CERT_MODE="sslip"
    fi
fi
...
case "$CERT_MODE" in
    ...
    sslip)
        if auravox_is_local_ipv4 "$SERVER_IP"; then
            auravox_fail "CERT_MODE=sslip needs a public IP; $SERVER_IP is private/reserved."
        fi
        command -v docker >/dev/null 2>&1 || auravox_fail "CERT_MODE=sslip needs Docker to serve the ACME challenge."
        ;;
    letsencrypt-dns|cloudflare-tunnel|external)
        auravox_fail "CERT_MODE=$CERT_MODE is reserved but not implemented yet. Use 'sslip' (public IP) or 'self-signed'."
        ;;
    *)
        auravox_fail "Unknown CERT_MODE '$CERT_MODE' (expected: auto, sslip, self-signed)."
        ;;
esac

if [[ "$CERT_MODE" == "sslip" ]]; then
    PUBLIC_HOST_VALUE="$(auravox_sslip_host_from_ip "$SERVER_IP" "$ACME_DOMAIN_SUFFIX")"
...
if [[ -z "${DEPLOY_MODE:-}" ]]; then
    if [[ -t 0 ]]; then
        ...
        echo "  1) prebuilt - pull official auravox images (recommended, fastest)"
        ...
        case "$mode_choice" in
            ...
            *) auravox_fail "invalid choice '$mode_choice'" ;;
        esac
    else
    ...
    if [[ "$REPO_SOURCE" == "clone" ]]; then
        if [[ -z "${FORK_REPO:-}" ]]; then
            if [[ -t 0 ]]; then
                ...
                read -p "[auravox-hq/auravox]: " FORK_REPO
                FORK_REPO="${FORK_REPO:-auravox-hq/auravox}"
            else
                FORK_REPO="auravox-hq/auravox"
            fi
        fi
...
[[ "$FASTAPI_WORKERS" =~ ^[1-9][0-9]*$ ]] || auravox_fail "FASTAPI_WORKERS must be a positive integer (got: $FASTAPI_WORKERS)"
...
if [[ "${AURAVOX_FORCE_OVERWRITE:-}" != "1" && "${AURAVOX_SKIP_DOWNLOAD:-}" != "1" ]]; then
    if [[ -f "$TARGET_DIR/.env" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Detected an existing Auravox install:${NC}"
        ...
        echo -e "  ${BLUE}https://docs.auravox.ai/deployment/update${NC}"
        echo ""
        echo -e "${BLUE}To wipe state and reinstall from scratch, re-run with:${NC}"
        echo -e "  ${BLUE}AURAVOX_FORCE_OVERWRITE=1 <same command>${NC}"
        echo ""
        exit 1
    fi
fi
...
if [[ "$DEPLOY_MODE" == "build" ]]; then
    if [[ "${AURAVOX_SKIP_DOWNLOAD:-}" == "1" ]]; then
        echo -e "${BLUE}[1/$TOTAL] Using existing repo in current directory${NC}"
    elif [[ "${REPO_SOURCE:-}" == "clone" ]]; then
        if [[ -e "auravox" ]]; then
            auravox_fail "'auravox' directory already exists. Remove it or re-run with REPO_SOURCE=existing from inside it."
        fi
        echo -e "${BLUE}[1/$TOTAL] Cloning $FORK_REPO (branch: $BRANCH)...${NC}"
        git clone --branch "$BRANCH" --recurse-submodules "https://github.com/$FORK_REPO.git" auravox
        cd auravox
    ...
    if [[ "${AURAVOX_SKIP_DOWNLOAD:-}" != "1" ]]; then
        mkdir -p auravox 2>/dev/null || true
        cd auravox

        echo -e "${BLUE}[1/$TOTAL] Downloading deployment bundle...${NC}"
        curl -fsSL -o docker-compose.yaml "https://raw.githubusercontent.com/auravox-hq/auravox/main/docker-compose.yaml"
        auravox_download_remote_support_bundle "$(pwd)" "main"
...
AURAVOX_DEPLOY_PROJECT_DIR="$(pwd)"
...
MINIO_ROOT_USER="auravox$(openssl rand -hex 6)"
...
auravox_prepare_remote_install "$(pwd)"
...
services:
  api:
    ...
    image: auravox-local/auravox-api:local
    pull_policy: never
  ...
  ui:
    ...
    image: auravox-local/auravox-ui:local
    pull_policy: never
OVERRIDE_EOF
...
if [[ "$CERT_MODE" == "sslip" ]]; then
    echo ""
    echo -e "${BLUE}Starting Auravox and requesting a trusted certificate for ${PUBLIC_HOST_VALUE}...${NC}"
    ...
    if [[ "$nginx_ready" != "1" ]]; then
        ...
        auravox_warn "nginx did not become reachable on port 80 — skipping Let's Encrypt for now."
        auravox_warn "The stack is running with the bootstrap self-signed certificate."
    elif auravox_install_certbot && auravox_issue_letsencrypt_webroot "$(pwd)" "$PUBLIC_HOST_VALUE" "$LETSENCRYPT_EMAIL"; then
        docker compose --profile remote restart nginx >/dev/null 2>&1 || true
        auravox_install_cert_renewal_hook "$(pwd)" "$PUBLIC_HOST_VALUE"
        CERT_RESULT="sslip"
        auravox_success "✓ Trusted Let's Encrypt certificate installed; auto-renewal configured"
    else
        CERT_RESULT="self-signed"
        echo ""
        auravox_warn "Let's Encrypt issuance failed — the stack is running with the self-signed certificate."
        auravox_warn "Common causes and fixes:"
        auravox_warn "  - Port 80 not reachable from the internet: open it in your firewall/security group"
        auravox_warn "  - Rate limited on ${ACME_DOMAIN_SUFFIX}: re-run with ACME_DOMAIN_SUFFIX=nip.io"
        auravox_warn "  - Then retry: sudo certbot certonly --webroot -w \"$(pwd)/certs\" -d ${PUBLIC_HOST_VALUE}"
    fi
fi
...
echo "  - remote_up.sh"
echo "  - scripts/run_auravox_init.sh"
...
if [[ "$CERT_MODE" == "sslip" ]]; then
    if [[ "$CERT_RESULT" == "sslip" ]]; then
        echo -e "${GREEN}Auravox is running with a trusted certificate at:${NC}"
        echo ""
        ...
        echo -e "${YELLOW}Auravox is running (with a temporary self-signed certificate) at:${NC}"
        echo ""
        echo -e "  ${BLUE}https://$PUBLIC_HOST_VALUE${NC}"
        echo ""
    ...
    echo -e "${YELLOW}To start Auravox, run:${NC}"
    echo ""
    if [[ "$DEPLOY_MODE" != "build" || "${REPO_SOURCE:-}" != "existing" ]]; then
