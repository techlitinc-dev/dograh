...
$UpstreamUrl    = 'https://github.com/auravox-hq/auravox.git'
$CanonicalHttps = $UpstreamUrl
$CanonicalSsh   = 'git@github.com:auravox-hq/auravox.git'
...
Write-Host '|              Auravox Contributor Bootstrap                    |' -ForegroundColor Blue
Write-Host '+==============================================================+' -ForegroundColor Blue
Write-Host ''
...
if ($needsForkPrompt) {
    Write-Host 'Enter your fork URL (e.g. https://github.com/<YOUR_HANDLE>/auravox.git):' -ForegroundColor Yellow
...
if (-not $existingUpstream) {
    ...
    $reset = (Read-Host 'Reset upstream to auravox-hq/auravox? [y/N]').Trim()
    if ($reset -match '^[Yy]') {
