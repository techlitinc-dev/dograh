#!/usr/bin/env pwsh
# Start Auravox services in development mode (Windows)
...
$EnvFile    = if ($env:AURAVOX_ENV_FILE) { $env:AURAVOX_ENV_FILE } else { Join-Path $BaseDir 'api/.env' }
...
Write-Host "Starting Auravox Services (DEV MODE) in BASE_DIR: $BaseDir"
