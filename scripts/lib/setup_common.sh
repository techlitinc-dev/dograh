...
AURAVOX_DEPLOY_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AURAVOX_DEPLOY_REPO_ROOT="$(cd "$AURAVOX_DEPLOY_LIB_DIR/../.." 2>/dev/null && pwd || true)"
...
auravox_project_dir() {
    if [[ -n "${AURAVOX_DEPLOY_PROJECT_DIR:-}" ]]; then
        printf '%s\n' "$AURAVOX_DEPLOY_PROJECT_DIR"
    else
        pwd
    fi
}

auravox_template_path() {
    local template_name=$1
    local candidate=""
    local project_dir

    project_dir="$(auravox_project_dir)"

    for candidate in \
        "$project_dir/deploy/templates/$template_name" \
        "$AURAVOX_DEPLOY_REPO_ROOT/deploy/templates/$template_name"
    do
        if [[ -f "$candidate" ]]; then
            printf '%s\n' "$candidate"
            return 0
    ...
    auravox_fail "Template '$template_name' not found"
}

auravox_init_script_path() {
    local candidate=""
    local project_dir
    ...
    for candidate in \
        "$project_dir/scripts/run_auravox_init.sh" \
        "$AURAVOX_DEPLOY_REPO_ROOT/scripts/run_auravox_init.sh"
    do
        if [[ -f "$candidate" ]]; then
            printf '%s\n' "$candidate"
            return 0
    ...
    auravox_fail "run_auravox_init.sh not found"
}

auravox_load_env_file() {
    local env_file=${1:-.env}

    [[ -f "$env_file" ]] || auravox_fail "$env_file not found"
...
auravox_is_local_ipv4() {
    local ip=$1
    local o1 o2 o3 o4 octet

    auravox_is_ipv4 "$ip" || return 1
...
auravox_infer_server_ip() {
    local project_dir=${1:-$(auravox_project_dir)}
    ...
    if [[ -n "${TURN_HOST:-}" ]] && auravox_is_ipv4 "$TURN_HOST"; then
        printf '%s\n' "$TURN_HOST"
        return 0
    fi

    if [[ -n "${PUBLIC_HOST:-}" ]] && auravox_is_ipv4 "$PUBLIC_HOST"; then
        printf '%s\n' "$PUBLIC_HOST"
        return 0
    fi
...
auravox_infer_public_host() {
    ...
    public_base_url="$(auravox_infer_public_base_url 2>/dev/null || true)"
    if [[ -n "$public_base_url" ]]; then
        auravox_host_from_url "$public_base_url"
        return 0
    fi
...
auravox_sync_remote_env_file() {
    ...
    auravox_load_env_file "$env_file"

    public_base_url="$(auravox_infer_public_base_url)" || auravox_fail "Could not determine PUBLIC_BASE_URL"
    public_base_url="${public_base_url%/}"
    public_host="$(auravox_infer_public_host)" || auravox_fail "Could not determine PUBLIC_HOST"
    server_ip="$(auravox_infer_server_ip "$project_dir")" || auravox_fail "Could not determine SERVER_IP"

    [[ "$public_base_url" =~ ^https?:// ]] || auravox_fail "PUBLIC_BASE_URL must include http:// or https://"
    auravox_is_ipv4 "$server_ip" || auravox_fail "SERVER_IP must be an IPv4 address (got: $server_ip)"

    auravox_set_env_key "$env_file" ENVIRONMENT "${ENVIRONMENT:-production}"
    auravox_set_env_key "$env_file" SERVER_IP "$server_ip"
    auravox_set_env_key "$env_file" PUBLIC_HOST "$public_host"
    auravox_set_env_key "$env_file" PUBLIC_BASE_URL "$public_base_url"
    ...
    auravox_set_env_key "$env_file" ENABLE_COTURN true
...
auravox_validate_remote_runtime_env() {
    [[ "${FASTAPI_WORKERS:-}" =~ ^[1-9][0-9]*$ ]] || auravox_fail "FASTAPI_WORKERS must be a positive integer"
    [[ -n "${TURN_SECRET:-}" ]] || auravox_fail "TURN_SECRET is missing"
    [[ -n "${PUBLIC_HOST:-}" ]] || auravox_fail "PUBLIC_HOST is missing"
    [[ -n "${PUBLIC_BASE_URL:-}" ]] || auravox_fail "PUBLIC_BASE_URL is missing"
    auravox_is_ipv4 "${SERVER_IP:-}" || auravox_fail "SERVER_IP must be a valid IPv4 address"
    [[ "${PUBLIC_BASE_URL}" =~ ^https?:// ]] || auravox_fail "PUBLIC_BASE_URL must include http:// or https://"
    # BACKEND_API_ENDPOINT / MINIO_PUBLIC_ENDPOINT / TURN_HOST are derived in-app
...
auravox_uses_init_compose_layout() {
    local project_dir=${1:-$(auravox_project_dir)}
    local compose_file="$project_dir/docker-compose.yaml"

    [[ -f "$compose_file" ]] || return 1
    grep -q "auravox-init:" "$compose_file" \
        && grep -q "nginx-generated:/etc/nginx/conf.d:ro" "$compose_file" \
        && grep -q "coturn-generated:/etc/coturn:ro" "$compose_file"
}

auravox_require_init_compose_layout() {
    local project_dir=${1:-$(auravox_project_dir)}

    if ! auravox_uses_init_compose_layout "$project_dir"; then
        auravox_fail "This install uses the legacy remote compose layout. Run ./update_remote.sh first so Docker uses auravox-init generated config."
    fi
}

auravox_render_remote_nginx_conf() {
    local project_dir=${1:-$(auravox_project_dir)}
    local destination=${2:-"$project_dir/nginx.conf"}
    local template=""
    local tmp_upstream=""

    template="$(auravox_template_path "nginx.remote.conf.template")"
    tmp_upstream="$(mktemp)"

    {
        echo "# Backend API workers - one uvicorn process per port, balanced by least_conn."
        echo "# Auto-generated by Auravox remote config renderer. Do not edit manually."
        echo "upstream auravox_api {"
        echo "    least_conn;"
    ...
    awk -v public_host="$PUBLIC_HOST" -v upstream_file="$tmp_upstream" '
        ...
        {
            gsub(/__AURAVOX_PUBLIC_HOST__/, public_host)
            if ($0 == "__AURAVOX_UPSTREAM_BLOCK__") {
                printf "%s", upstream
            } else {
                print
            }
...
auravox_render_remote_turn_conf() {
    local project_dir=${1:-$(auravox_project_dir)}
    local destination=${2:-"$project_dir/turnserver.conf"}
    local template=""
    local external_ip="${TURN_EXTERNAL_IP:-${SERVER_IP:-}}"

    template="$(auravox_template_path "turnserver.remote.conf.template")"
    [[ -n "$external_ip" ]] || auravox_fail "TURN external IP/host is missing"
    ...
    awk \
        ...
        '
        {
            gsub(/__AURAVOX_TURN_EXTERNAL_IP__/, external_ip)
            gsub(/__AURAVOX_TURN_SECRET__/, turn_secret)
            print
        }
    ' "$template" > "$destination"
}

auravox_preflight_remote_init_render() {
    local project_dir=${1:-$(auravox_project_dir)}
    ...
    auravox_load_env_file "$env_file"
    auravox_validate_remote_runtime_env
    [[ -f "$cert_dir/local.crt" ]] || auravox_fail "certs/local.crt not found"
    [[ -f "$cert_dir/local.key" ]] || auravox_fail "certs/local.key not found"

    init_script="$(auravox_init_script_path)"
    tmp_root="$(mktemp -d)"
    ...
    (
        ...
        export AURAVOX_INIT_WORKSPACE_DIR="$project_dir"
        export AURAVOX_INIT_OUTPUT_ROOT="$tmp_root"
        export AURAVOX_INIT_CERTS_DIR="$cert_dir"
        bash "$init_script" >/dev/null
    )

    [[ -f "$nginx_conf" ]] || auravox_fail "auravox-init did not render nginx config"
    [[ -f "$turn_conf" ]] || auravox_fail "auravox-init did not render coturn config"

    nginx_workers=$(awk '/^[[:space:]]*server api:[0-9]+/ { count += 1 } END { print count + 0 }' "$nginx_conf")
    [[ "$nginx_workers" -eq "$FASTAPI_WORKERS" ]] || auravox_fail "FASTAPI_WORKERS=$FASTAPI_WORKERS but nginx.conf has $nginx_workers upstream servers"

    rendered_server_name="$(awk '/^[[:space:]]*server_name / { print $2; exit }' "$nginx_conf" | sed 's/;$//')"
    [[ "$rendered_server_name" == "$PUBLIC_HOST" ]] || auravox_fail "nginx.conf server_name ($rendered_server_name) does not match PUBLIC_HOST ($PUBLIC_HOST)"

    rendered_secret="$(sed -n 's/^static-auth-secret=//p' "$turn_conf" | head -1)"
    [[ "$rendered_secret" == "$TURN_SECRET" ]] || auravox_fail "TURN_SECRET in .env does not match turnserver.conf"

    rendered_ip="$(sed -n 's/^external-ip=//p' "$turn_conf" | head -1)"
    [[ "$rendered_ip" == "$SERVER_IP" ]] || auravox_fail "SERVER_IP in .env does not match turnserver.conf"
...
auravox_sync_postgres_password() {
    local project_dir=$1
    shift
    local compose=("$@")
    local env_file="$project_dir/.env"
    ...
    auravox_info "Syncing Postgres password from .env..."
    ...
    [[ -n "$ready" ]] || auravox_fail "Postgres did not become ready while syncing POSTGRES_PASSWORD."
    ...
    printf '%s\n' "ALTER USER postgres WITH PASSWORD :'pw';" \
        ...
        || auravox_fail "Failed to sync Postgres password from .env."
    auravox_success "✓ Postgres password synced with .env"
}

auravox_prepare_remote_install() {
    local project_dir=${1:-$(auravox_project_dir)}
    local env_file="$project_dir/.env"

    auravox_sync_remote_env_file "$env_file"
    auravox_require_init_compose_layout "$project_dir"
    auravox_preflight_remote_init_render "$project_dir"
}
...
auravox_sslip_host_from_ip() {
    local ip=$1
    local suffix=${2:-sslip.io}

    auravox_is_ipv4 "$ip" || auravox_fail "auravox_sslip_host_from_ip: '$ip' is not an IPv4 address"
    printf '%s.%s\n' "${ip//./-}" "$suffix"
}
...
auravox_install_certbot() {
    if command -v certbot >/dev/null 2>&1; then
        return 0
    fi

    auravox_info "Installing Certbot..."
    if command -v apt-get >/dev/null 2>&1; then
        ...
        auravox_warn "Could not detect a package manager (apt/dnf/yum) to install certbot."
        return 1
    fi
}
...
auravox_issue_letsencrypt_webroot() {
    local project_dir=$1
    local host=$2
    local email=${3:-}
...
auravox_install_cert_renewal_hook() {
    local project_dir=$1
    local host=$2
    local hook_dir="/etc/letsencrypt/renewal-hooks/deploy"
    local hook_path="$hook_dir/auravox-reload.sh"
...
auravox_download_bundle_file_for_ref() {
    local destination=$1
    local remote_path=$2
    local ref=${3:-main}
    local raw_base="https://raw.githubusercontent.com/auravox-hq/auravox/$ref"
    local fallback_base="https://raw.githubusercontent.com/auravox-hq/auravox/main"

    if ! curl -fsSL -o "$destination" "$raw_base/$remote_path"; then
        auravox_warn "Warning: '$remote_path' not found at '$ref' - falling back to main"
        curl -fsSL -o "$destination" "$fallback_base/$remote_path"
    fi
}

auravox_download_init_support_bundle() {
    local project_dir=$1
    local ref=${2:-main}
    ...
    auravox_download_bundle_file_for_ref "$project_dir/scripts/lib/setup_common.sh" "scripts/lib/setup_common.sh" "$ref"
    auravox_download_bundle_file_for_ref "$project_dir/scripts/run_auravox_init.sh" "scripts/run_auravox_init.sh" "$ref"
    chmod +x "$project_dir/scripts/run_auravox_init.sh"
    auravox_download_bundle_file_for_ref "$project_dir/deploy/templates/nginx.remote.conf.template" "deploy/templates/nginx.remote.conf.template" "$ref"
    auravox_download_bundle_file_for_ref "$project_dir/deploy/templates/turnserver.remote.conf.template" "deploy/templates/turnserver.remote.conf.template" "$ref"
}

auravox_download_remote_support_bundle() {
    local project_dir=$1
    local ref=${2:-main}

    auravox_download_bundle_file_for_ref "$project_dir/remote_up.sh" "remote_up.sh" "$ref"
    chmod +x "$project_dir/remote_up.sh"
    auravox_download_init_support_bundle "$project_dir" "$ref"
}
