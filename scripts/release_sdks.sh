#!/usr/bin/env bash
# Cut a release of both SDKs — auravox-sdk (PyPI) and @auravox/sdk (npm) —
...
#   - `npm login` completed as a member of the `auravox` npm org. npm
...
if ! NPM_USER="$(npm whoami 2>/dev/null)"; then
    ...
    echo "       auravox org before re-running this script — otherwise PyPI" >&2
...
if ! git diff --quiet -- sdk/python/src/auravox_sdk/typed sdk/typescript/src/typed; then
    echo
...
echo
echo "============================================================"
echo "  Built auravox-sdk==$VERSION and @auravox/sdk@$VERSION"
echo "  Nothing has been published yet."
echo "============================================================"
echo

if confirm "Upload auravox-sdk==$VERSION to TestPyPI first (recommended)?"; then
    (cd sdk/python && twine upload --repository testpypi dist/*)
    echo "  → https://test.pypi.org/project/auravox-sdk/$VERSION/"
    echo
fi

if confirm "Publish @auravox/sdk@$VERSION to npm? (will prompt for 2FA OTP)"; then
    (cd sdk/typescript && npm publish --access public)
    echo "  → https://www.npmjs.com/package/@auravox/sdk/v/$VERSION"
    echo
fi

if confirm "Upload auravox-sdk==$VERSION to PyPI?"; then
    (cd sdk/python && twine upload dist/*)
    echo "  → https://pypi.org/project/auravox-sdk/$VERSION/"
    echo
fi

if confirm "Create annotated git tag sdks-v$VERSION at HEAD?"; then
    git tag -a "sdks-v$VERSION" -m "auravox-sdk + @auravox/sdk $VERSION"
