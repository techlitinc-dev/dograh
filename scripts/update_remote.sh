...
generate_secret() {
    ...
    auravox_fail "Could not generate a secret. Install python3 or openssl, or set missing secrets manually in .env."
}
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                  Auravox Remote Update                        ║"
echo "║  Refresh deployment files and validate runtime config        ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

[[ -f docker-compose.yaml ]] || auravox_fail "docker-compose.yaml not found in $(pwd)"
[[ -f .env ]] || auravox_fail ".env not found in $(pwd)"
...
if [[ -f docker-compose.override.yaml ]]; then
    ...
    echo ""
    echo -e "${YELLOW}See https://docs.auravox.ai/deployment/update#updating-a-source-build${NC}"
    exit 1
fi
...
auravox_load_env_file .env

[[ -n "${TURN_SECRET:-}" ]] || auravox_fail "TURN_SECRET not found in .env"
...
if [[ -z "$TARGET_VERSION" ]]; then
    auravox_info "Fetching latest release tag from GitHub..."
    LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)

    if [[ -z "$LATEST_TAG" ]]; then
        auravox_warn "Could not auto-discover latest tag - defaulting to 'main'."
        LATEST_TAG="main"
    fi
    ...
    if [[ -t 0 ]]; then
        ...
        echo -e "${YELLOW}full git tag (auravox-v1.28.0), or 'main' for the latest deployment files.${NC}"
...
if [[ "$TARGET_VERSION" == "latest" ]]; then
    TARGET_VERSION=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)
    [[ -n "$TARGET_VERSION" ]] || auravox_fail "could not resolve 'latest' to a release tag"
fi
...
case "$TARGET_VERSION" in
    ...
    auravox-*)
        ;;
    v*)
        TRY_TAGS+=("auravox-$TARGET_VERSION")
        ;;
    *)
        TRY_TAGS+=("auravox-v$TARGET_VERSION" "v$TARGET_VERSION" "auravox-$TARGET_VERSION")
        ;;
esac

auravox_info "Validating target version: $TARGET_VERSION..."
RESOLVED_TAG=""
...
[[ -n "$RESOLVED_TAG" ]] || auravox_fail "could not find a git tag matching '$TARGET_VERSION'"

if [[ "$RESOLVED_TAG" != "$TARGET_VERSION" ]]; then
    auravox_success "✓ Resolved '$TARGET_VERSION' to git tag '$RESOLVED_TAG'"
fi
...
case "$TARGET_VERSION" in
    auravox-v*) IMAGE_TAG="${TARGET_VERSION#auravox-v}" ;;
...
if [[ -n "$IMAGE_TAG" ]]; then
    if curl -fsI "https://hub.docker.com/v2/repositories/auravoxai/auravox-api/tags/$IMAGE_TAG/" >/dev/null 2>&1; then
        auravox_success "✓ Image tag :$IMAGE_TAG found on Docker Hub"
    else
        auravox_warn "Warning: image tag :$IMAGE_TAG not found on Docker Hub - leaving images at :latest"
        IMAGE_TAG=""
    fi
fi
...
echo -e "  Server IP:        ${BLUE}$(auravox_infer_server_ip "$(pwd)" || echo "unknown")${NC}"
...
if [[ -t 0 && "${AURAVOX_UPDATE_YES:-}" != "1" ]]; then
...
auravox_download_remote_support_bundle "$(pwd)" "$TARGET_VERSION"
rm -f nginx.conf turnserver.conf

if [[ -n "$IMAGE_TAG" ]]; then
    sed -i.tmp -E "s#(auravox-(api|ui)):latest#\1:$IMAGE_TAG#g" docker-compose.yaml
    rm -f docker-compose.yaml.tmp
    auravox_success "✓ docker-compose.yaml updated; images pinned to :$IMAGE_TAG"
else
    auravox_success "✓ docker-compose.yaml updated (image tags left at :latest)"
fi

echo -e "${BLUE}[3/3] Synchronizing environment and validating init-based remote config...${NC}"
auravox_set_env_key .env FASTAPI_WORKERS "$FASTAPI_WORKERS"
if [[ -z "${REDIS_PASSWORD:-}" ]]; then
    auravox_set_env_key .env REDIS_PASSWORD "$(generate_secret)"
    auravox_success "✓ REDIS_PASSWORD created in .env"
fi
if [[ -z "${MINIO_ROOT_USER:-}" ]]; then
    if [[ -n "${MINIO_ACCESS_KEY:-}" ]]; then
        auravox_set_env_key .env MINIO_ROOT_USER "$MINIO_ACCESS_KEY"
        auravox_success "✓ MINIO_ROOT_USER created in .env from existing MINIO_ACCESS_KEY"
    else
        auravox_set_env_key .env MINIO_ROOT_USER "$(generate_minio_root_user)"
        auravox_success "✓ MINIO_ROOT_USER created in .env"
    fi
fi
if [[ -z "${MINIO_ROOT_PASSWORD:-}" ]]; then
    if [[ -n "${MINIO_SECRET_KEY:-}" ]]; then
        auravox_set_env_key .env MINIO_ROOT_PASSWORD "$MINIO_SECRET_KEY"
        auravox_success "✓ MINIO_ROOT_PASSWORD created in .env from existing MINIO_SECRET_KEY"
    else
        auravox_set_env_key .env MINIO_ROOT_PASSWORD "$(generate_secret)"
        auravox_success "✓ MINIO_ROOT_PASSWORD created in .env"
    fi
fi
auravox_prepare_remote_install "$(pwd)"
docker compose config -q
auravox_success "✓ Remote init configuration validated"
...
echo ""
echo -e "  ${BLUE}for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_auravox_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do${NC}"
