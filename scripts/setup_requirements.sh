...
AURAVOX_DIR="$(dirname "$SCRIPT_DIR")"
...
# Install auravox API requirements first so pipecat's extras win on any
# shared transitive dependencies (matches api/Dockerfile and CI workflow).
echo "Installing auravox API requirements..."
uv pip install -r api/requirements.txt

if [ "$DEV_MODE" -eq 1 ]; then
    echo "Installing auravox API dev requirements..."
