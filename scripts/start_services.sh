...
# Log startup
cd "$BASE_DIR"
echo "Starting Auravox Services at $(date) in BASE_DIR: ${BASE_DIR}"
...
if [[ -z "${AURAVOX_DEVOPS_SECRET:-}" ]]; then
  echo "ERROR: AURAVOX_DEVOPS_SECRET is not set. Add it to $ENV_FILE before starting production services."
  exit 1
fi
if [[ "$AURAVOX_DEVOPS_SECRET" == "change-me-auravox-devops-secret" ]]; then
  echo "ERROR: AURAVOX_DEVOPS_SECRET still has the example placeholder value. Replace it in $ENV_FILE."
  exit 1
fi
