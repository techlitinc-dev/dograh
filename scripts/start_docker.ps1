...
function New-MinioRootUser {
    return "auravox$((New-HexSecret).Substring(0, 12))"
}
...
function Sync-PostgresPassword {
    ...
    "ALTER USER postgres WITH PASSWORD :'auravox_password';" | docker compose exec -T postgres psql `
        -U postgres `
        -d postgres `
        -v 'ON_ERROR_STOP=1' `
        -v "auravox_password=$Password" > $null
    if ($LASTEXITCODE -ne 0) {
...
$answer = Read-Host 'Start Auravox now? [Y/n]'
if ($answer -match '^[Nn]') {
    Write-Host 'Auravox was not started.'
    exit 0
}
