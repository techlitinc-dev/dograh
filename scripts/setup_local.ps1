...
function Download-BundleFileForRef([string]$Destination, [string]$RemotePath, [string]$Ref) {
    $rawBase = "https://raw.githubusercontent.com/auravox-hq/auravox/$Ref"
    $fallbackBase = 'https://raw.githubusercontent.com/auravox-hq/auravox/main'
...
function Download-InitSupportBundle([string]$ProjectDir, [string]$Ref) {
    ...
    Download-BundleFileForRef (Join-Path $ProjectDir 'scripts/run_auravox_init.sh') 'scripts/run_auravox_init.sh' $Ref
...
Write-Info ''
Write-Info '╔══════════════════════════════════════════════════════════════╗'
Write-Info '║                    Auravox Local Setup                        ║'
...
$Registry = if ([string]::IsNullOrEmpty($env:REGISTRY)) { 'ghcr.io/auravox-hq' } else { $env:REGISTRY }
...
if ($env:AURAVOX_SKIP_DOWNLOAD -ne '1') {
    if ($UseCoturn) {
    ...
    Download-File 'https://raw.githubusercontent.com/auravox-hq/auravox/main/docker-compose.yaml' (Join-Path $CurrentDir 'docker-compose.yaml')
    if ($UseCoturn) {
...
if ($UseCoturn) {
    Assert-PathExists 'scripts/run_auravox_init.sh' 'scripts/run_auravox_init.sh not found. Re-run setup_local.ps1 without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'scripts/lib/setup_common.sh' 'scripts/lib/setup_common.sh not found. Re-run setup_local.ps1 without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'deploy/templates/turnserver.remote.conf.template' 'deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.ps1 without AURAVOX_SKIP_DOWNLOAD=1, or use a full repo checkout.'
}
...
$minioRootUser = "auravox$((New-HexSecret 6).Substring(0, 12))"
$minioRootPassword = New-HexSecret 32

$envLines = @(
    '# Container registry for Auravox images'
    "REGISTRY=$Registry"
    ''
