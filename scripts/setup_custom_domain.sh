...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              Auravox Custom Domain Setup                      ║"
echo "║     Automated Let's Encrypt SSL certificate setup            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

if [[ $EUID -ne 0 ]]; then
    auravox_fail "This script must be run as root or with sudo"
fi

if [[ ! -d "auravox" ]]; then
    echo -e "${RED}Error: 'auravox' directory not found.${NC}"
    echo -e "${YELLOW}Please run this script from the directory containing your Auravox installation.${NC}"
    echo -e "${YELLOW}If you haven't set up Auravox yet, run the remote setup first:${NC}"
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/auravox-hq/auravox/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && sudo ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || auravox_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    auravox_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || auravox_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    auravox_warn "Warning: Could not detect server's public IP"
...
auravox_install_certbot || auravox_fail "Could not install certbot. Please install it manually and re-run."
...
auravox_require_init_compose_layout "$(pwd)"

auravox_load_env_file .env
if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(auravox_infer_server_ip "$(pwd)" || true)"
fi
[[ -n "${SERVER_IP:-}" ]] || auravox_fail "Could not determine SERVER_IP from the existing install"

auravox_set_env_key .env SERVER_IP "$SERVER_IP"
auravox_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
auravox_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
auravox_delete_env_key .env BACKEND_URL
...
auravox_delete_env_key .env BACKEND_API_ENDPOINT
auravox_delete_env_key .env MINIO_PUBLIC_ENDPOINT
auravox_delete_env_key .env TURN_HOST
auravox_prepare_remote_install "$(pwd)"

# Bring the stack up (recreating it) so auravox-init re-renders nginx with the
...
[[ "$nginx_ready" == "1" ]] || auravox_fail "nginx did not come up on port 80; cannot run the ACME challenge."
...
if ! auravox_issue_letsencrypt_webroot "$(pwd)" "$DOMAIN_NAME" "$EMAIL_ADDRESS"; then
...
auravox_install_cert_renewal_hook "$(pwd)" "$DOMAIN_NAME"
...
echo -e "  Certificate: $AURAVOX_PATH/certs/local.crt"
echo -e "  Private Key: $AURAVOX_PATH/certs/local.key"
echo -e "  Auto-renewal: Enabled (certificates renew automatically)"
echo ""
echo -e "${YELLOW}Files modified:${NC}"
echo "  - auravox/.env (canonical public host/base URL updated)"
echo "  - auravox/certs/local.crt (SSL certificate)"
echo "  - auravox/certs/local.key (SSL private key)"
echo "  - /etc/letsencrypt/renewal-hooks/deploy/auravox-reload.sh (renewal hook)"
echo ""
