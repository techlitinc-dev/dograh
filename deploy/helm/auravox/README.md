...
Deploys Auravox on Kubernetes with decomposed backend workloads (web,
...
|------------------------------|-------------|-----------------|-------|
| `auravox-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `auravox-arq-worker`          | 1 (HPA opt) | RollingUpdate   | Stateless |
| `auravox-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `auravox-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `auravox-ui`                  | 2 (HPA opt) | RollingUpdate   | Next.js SSR |
| `auravox-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
