...
_FAILURE_METADATA_ATTR = "_auravox_failure_metadata"
_FAILURE_REPORTED_ATTR = "_auravox_failure_reported"
...
def _resolve_error_owner(
    ...
    "user" and made every unclassifiable provider error look like a Auravox bug.

    An unstated owner still falls back to operator — an unattributed failure is
    Auravox's to investigate until a seam claims otherwise.
    """
...
def _external_message(source: ErrorSource, error_type: ErrorType) -> str:
    ...
    return f"Auravox encountered an internal error while processing {label}."
...
def classify_http_response(
    ...
    *,
    ...
    if normalized_provider == "auravox":
        if status_code == 402:
...
def failure_metadata_for_processor(processor: object | None) -> ServiceFailureMetadata:
    ...
    # A Auravox-prefixed wrapper is not sufficient evidence that the dependency is
    ...
    **extra_context: Any,
    ...
    try:
        ...
        bound.opt(depth=1).log(
            level.upper(),
            "AURAVOX_FAILURE [src={} type={} code={}] [owner={}] {}",
