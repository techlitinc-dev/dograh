...
async def create_mps_credit_purchase_url(
    ...
    try:
        session = await mps_service_key_client.create_credit_purchase_url(
            ...
            billing_details={
                "source": "auravox_billing",
                "auravox_user_id": str(user.id),
                "auravox_provider_id": str(user.provider_id),
            },
        )
    except Exception as exc:
...
async def get_usage_history(
    ...
    try:
        ...
        return {
            "runs": runs,
            "total_auravox_tokens": total_tokens,
