"""Pydantic schemas for reusable Auravox tools.
...
class PresetToolParameter(BaseModel):
    """A parameter injected by Auravox at runtime."""
...
class HttpApiConfig(BaseModel):
    ...
    preset_parameters: list[PresetToolParameter] | None = Field(
        default=None,
        description=(
            "Parameters injected by Auravox from fixed values or workflow context "
            "templates."
        ),
    )
...
class HttpTransferResolverConfig(BaseModel):
    ...
    wait_message: str | None = Field(
        default=None,
        description="Optional short message played while Auravox resolves routing.",
    )
    parameters: list[ToolParameter] | None = Field(
        default=None,
