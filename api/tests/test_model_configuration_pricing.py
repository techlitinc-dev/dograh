...
async def test_model_configuration_pricing_returns_empty_in_oss(monkeypatch):
    ...
    assert response.auravox_model is None
...
async def test_model_configuration_pricing_uses_selected_organization(monkeypatch):
    get_pricing = AsyncMock(
        return_value={
            ...
            "auravox_model": {
                ...
                "display_name": "Auravox model usage",
                "unit": "minute",
    ...
    assert response.auravox_model is not None
    assert response.auravox_model.price_per_minute == 0.07
