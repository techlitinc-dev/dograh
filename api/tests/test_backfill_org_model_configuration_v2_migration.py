...
def test_auravox_legacy_converts_to_managed_mode():
    value = migration.convert_legacy_configuration_to_v2(
        {
            "llm": {"provider": "auravox", "api_key": "mps-key", "model": "default"},
            "tts": {
                "provider": "auravox",
                "api_key": "mps-key",
                "voice": "aura",
            ...
            "stt": {"provider": "auravox", "api_key": "mps-key", "language": "en"},
        }
    )
    assert value["mode"] == "auravox"
    assert value["auravox"] == {
        "api_key": "mps-key",
        "voice": "aura",
        "speed": 1.2,
        "language": "en",
...
def test_auravox_mode_wins_over_byok_sections_and_sanitizes_speed():
    value = migration.convert_legacy_configuration_to_v2(
        {
            "llm": {"provider": "auravox", "api_key": ["mps-key"]},
            "tts": {"provider": "elevenlabs", "api_key": "el-key", "speed": 9.0},
            "stt": {"provider": "deepgram", "api_key": "dg-key"},
        }
    )
    assert value["mode"] == "auravox"
    assert value["auravox"]["api_key"] == "mps-key"
    assert value["auravox"]["speed"] == 1.0
    _parse_and_compile(value)
...
def test_auravox_provider_without_single_key_cannot_become_byok():
    # Multiple auravox keys can't map to managed mode, and BYOK rejects the
    # auravox provider — the org must be skipped rather than written broken.
    assert (
        migration.convert_legacy_configuration_to_v2(
            {
                "llm": {"provider": "auravox", "api_key": ["key-a", "key-b"]},
                "tts": {"provider": "elevenlabs", "api_key": "el-key"},
                "stt": {"provider": "deepgram", "api_key": "dg-key"},
            }
        )
