...
def _auravox_config(
    ...
    *,
    ...
    return SimpleNamespace(
        ...
        llm=SimpleNamespace(provider=ServiceProviders.AURAVOX, api_key=api_key),
        stt=None,
        tts=None,
        embeddings=None,
    )
...
async def test_authorize_workflow_run_uses_workflow_org_for_hosted_v2(
    ...
    get_config = AsyncMock(return_value=_auravox_config())
    authorize = AsyncMock(
        return_value={
            "allowed": True,
            "billing_mode": "v2",
    ...
    authorize.assert_awaited_once_with(
        ...
        minimum_credits=quota_service.MINIMUM_AURAVOX_CREDITS_FOR_CALL,
        created_by="provider-123",
        metadata={"auravox_user_id": "123", "workflow_id": 7},
    )
...
async def test_authorize_workflow_run_v2_insufficient_credits_prompts_billing(
    ...
    assert "founders@auravox.ai" not in result.error_message
...
async def test_authorize_workflow_run_oss_exhausted_key_blocks_run(
    ...
    get_config = AsyncMock(return_value=_auravox_config(api_key))
    check_usage = AsyncMock(
    ...
    assert "app.auravox.ai" in result.error_message
...
async def test_authorize_workflow_run_managed_v2_stores_hosted_correlation(
    ...
    get_config = AsyncMock(
        return_value=_auravox_config(api_key, managed_service_version=2)
    )
    authorize = AsyncMock(
        return_value={
            "allowed": True,
...
async def test_hosted_managed_v2_authorization_rejects_missing_correlation(
    ...
    result = await quota_service._authorize_hosted_workflow_run_start(
        ...
        workflow_id=7,
        workflow_run_id=88,
        user_config=_auravox_config(managed_service_version=2),
    )
...
async def test_oss_run_authorization_falls_back_for_older_mps(monkeypatch):
    ...
    request = httpx.Request(
        "POST",
        "https://services.auravox.ai/api/v1/service-keys/run-authorization/self",
    )
    ...
    result = await quota_service._authorize_oss_managed_v2_run(
        ...
        workflow_run_id=88,
        service_key=api_key,
        user_config=_auravox_config(api_key, managed_service_version=2),
    )
...
async def test_authorize_workflow_run_resolves_config_from_pinned_definition(
    ...
    Auravox service key, minting from the workflow column binds the correlation
...
async def test_authorize_workflow_run_opens_when_hosted_mps_is_unreachable(
    ...
    request = httpx.Request(
        "POST",
        "https://services.auravox.ai/api/v1/billing/accounts/42/run-authorization",
    )
...
async def test_authorize_workflow_run_fails_closed_on_invalid_mps_url(monkeypatch):
    request = httpx.Request("POST", "ftp://services.auravox.ai/run-authorization")
...
async def test_authorize_workflow_run_opens_when_oss_quota_mps_is_unreachable(
    ...
    request = httpx.Request(
        "GET",
        "https://services.auravox.ai/api/v1/service-keys/usage/self",
    )
    ...
    monkeypatch.setattr(
        quota_service,
        "get_effective_ai_model_configuration_for_workflow",
        AsyncMock(return_value=_auravox_config()),
    )
    monkeypatch.setattr(
        quota_service.mps_service_key_client,
        "check_service_key_usage",
...
async def test_authorize_workflow_run_denies_when_oss_run_authorization_is_unreachable(
    ...
    monkeypatch.setattr(
        quota_service,
        "get_effective_ai_model_configuration_for_workflow",
        AsyncMock(return_value=_auravox_config(managed_service_version=2)),
    )
    monkeypatch.setattr(
        quota_service.mps_service_key_client,
        "authorize_service_key_run_start",
