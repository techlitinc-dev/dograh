...
def test_auravox_boundary_changes_attribution():
    quota = classify_http_response(
        402,
        "Insufficient Auravox credits",
        source=ErrorSource.LLM,
        provider="auravox",
    )
...
def test_auravox_credit_prose_does_not_override_safe_default():
    ...
    assert failure.code == "auravox-unknown"


def test_auravox_transport_failure_stays_system_error_but_keeps_retry_hint():
    failure = classify_exception(
    ...
    assert failure.type == ErrorType.SYSTEM_ERROR
    assert failure.error_owner == ErrorOwner.OPERATOR
    assert failure.code == "auravox-connection"
    assert failure.retryable is True
...
def test_factory_metadata_is_authoritative_over_wrapper_class_name():
    service = type("AuravoxOpenAIRealtimeLLMService", (), {})()
    annotate_failure_metadata(
        service,
        source=ErrorSource.LLM,
...
def test_log_failure_emits_structured_and_inline_classification():
    ...
    record = records[-1]
    assert "AURAVOX_FAILURE [src=webhook type=config_error code=webhook-404]" in str(
        record["message"]
    )
