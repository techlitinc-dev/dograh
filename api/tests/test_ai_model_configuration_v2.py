...
def test_auravox_v2_compiles_to_effective_managed_pipeline_with_embeddings():
    config = OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(
            api_key="mps-secret",
            voice="default",
            speed=1.2,
            language="multi",
    ...
    assert effective.llm.provider == "auravox"
    assert effective.llm.model == "default"
    assert effective.tts.provider == "auravox"
    assert effective.tts.speed == 1.2
    assert effective.stt.provider == "auravox"
    assert effective.stt.language == "multi"
    assert effective.embeddings.provider == "auravox"
    assert effective.embeddings.model == "auravox_embedding_v1"
    assert effective.managed_service_version == 2


def test_auravox_v2_accepts_numeric_speed_in_registry_range():
    config = OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(
            api_key="mps-secret",
...
def test_auravox_v2_rejects_out_of_range_speed():
...
def test_byok_v2_rejects_auravox_provider():
    with pytest.raises(ValidationError):
...
async def test_resolved_org_v2_uses_last_validated_at_as_validation_cache(
    ...
    config = OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(api_key="mps-secret"),
    )
    row = SimpleNamespace(
...
def test_masked_auravox_key_is_preserved_when_saving_same_mode():
    existing = OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(api_key="mps-real-secret"),
    )
    incoming = OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(api_key=mask_key("mps-real-secret")),
    )

    merged = merge_ai_model_configuration_v2_secrets(incoming, existing)

    assert merged.auravox.api_key == "mps-real-secret"
...
def test_legacy_all_auravox_pipeline_converts_to_auravox_v2():
    ...
    assert config.mode == "auravox"
    assert config.auravox.api_key == "mps-secret"


def test_legacy_auravox_pipeline_conversion_preserves_numeric_speed():
    legacy = EffectiveAIModelConfiguration(
        llm=AuravoxLLMService(
            provider="auravox",
            api_key=["mps-secret"],
    ...
    assert config.mode == "auravox"
    assert config.auravox.speed == 1.5


def test_legacy_mixed_auravox_pipeline_converts_to_auravox_v2():
    legacy = EffectiveAIModelConfiguration(
    ...
    assert config.mode == "auravox"
    assert config.auravox.api_key == "mps-tts"
    assert config.auravox.voice == "default"
...
def test_workflow_model_override_migration_removes_v1_override_and_sets_v2():
    ...
    assert v2_override["mode"] == "auravox"
    assert v2_override["auravox"]["api_key"] == "mps-workflow"
...
async def test_migrate_model_configuration_v2_initializes_hosted_mps_billing(
    ...
    expected_response = OrganizationAIModelConfigurationResponse(
        configuration={"version": 2, "mode": "auravox"},
