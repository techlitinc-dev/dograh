"""Tests for the Auravox-managed embedding service and its correlation resolver."""
...
from api.services.gen_ai.embedding.auravox_service import AuravoxEmbeddingService
...
def _service_with_fake_client(correlation_id):
    service = AuravoxEmbeddingService(
        db_client=None,
        api_key="sk-test",
...
@pytest.mark.asyncio
async def test_auravox_embedding_forwards_v2_protocol_when_correlation_present():
...
@pytest.mark.asyncio
async def test_auravox_embedding_sends_plain_without_correlation():
