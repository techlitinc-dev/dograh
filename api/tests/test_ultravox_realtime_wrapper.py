...
from api.services.pipecat.realtime.ultravox_realtime import (
    AuravoxUltravoxOneShotInputParams,
    AuravoxUltravoxRealtimeLLMService,
)
...
def _make_service() -> AuravoxUltravoxRealtimeLLMService:
    service = AuravoxUltravoxRealtimeLLMService(
        params=AuravoxUltravoxOneShotInputParams(
            api_key="test-key",
            model="ultravox-v0.7",
            output_medium="voice",
        ),
        settings=AuravoxUltravoxRealtimeLLMService.Settings(
            model="ultravox-v0.7",
            output_medium="voice",
        ),
    )
...
async def test_system_instruction_update_marks_stage_update_required():
    ...
    changed = await service._update_settings(
        AuravoxUltravoxRealtimeLLMService.Settings(system_instruction="new instruction")
    )
...
def test_build_one_shot_params_uses_explicit_greeting_text():
    ...
    params = service._build_one_shot_params(
        greeting_text="Welcome to Auravox",
        agent_speaks_first=True,
    )

    assert params.extra["firstSpeakerSettings"] == {
        "agent": {"text": "Welcome to Auravox"}
    }
...
def test_factory_creates_auravox_ultravox_realtime_service():
    ...
    assert isinstance(service, AuravoxUltravoxRealtimeLLMService)
