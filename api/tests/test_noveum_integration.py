...
def _completion_context(logs=None, recording_url="https://auravox/rec.wav"):
    return types.SimpleNamespace(
        workflow_run_id=42,
...
async def test_on_call_finished_returns_envelope_with_disposition():
    ...
    session = NoveumRuntimeSession(
        observer, manifest=manifest, call_attributes={"auravox.workflow_run_id": 42}
    )
    ...
    assert attrs["auravox.workflow_run_id"] == 42
    assert attrs["auravox.call_disposition"] == "completed"
...
async def test_completion_happy_path_uploads_audio_and_sends_trace():
    ...
    assert sent_trace["attributes"]["auravox.recording_url"] == "https://auravox/rec.wav"
...
async def test_completion_passes_agent_version_as_service_version():
    # auravox.agent_version stamped in the live phase must surface as the
    # completion client's service_version (stringified).
    envelope = {
        "schema_version": NOVEUM_SNAPSHOT_SCHEMA_VERSION,
        "trace": _trace_snapshot(**{"auravox.agent_version": 2}),
        "audio_manifest": [],
    }
...
async def test_envelope_sanitized_to_pure_json():
    ...
    # whole commit (including auravox's own keys) fail. default=str converts.
    ...
    envelope = build_payload_envelope(observer, [], {"auravox.workflow_run_id": 42})
