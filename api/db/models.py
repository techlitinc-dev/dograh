...
organization_users_association = Table(
    ...
    # Values come from OrganizationRole; existing rows were backfilled 'owner'.
...
class OrganizationModel(Base):
    ...
    provider_id = Column(String, unique=True, index=True, nullable=False)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
    # Human-readable display name; settable via PATCH /organizations.
...
class TelephonyTrunkModel(Base):
    ...
    Only providers whose Auravox integration declares ``trunk_settings_cls`` get
    ...
    a trunk in their own consoles, but Auravox drives them through their
    ...
    # The provider's own identifier for this trunk, recorded when Auravox
    # provisions it remotely. Kept out of ``settings`` because it is Auravox's
...
# Association table: CRM contacts <-> tags (org-scoped segmentation).
contact_tags_association = Table(
    "contact_tags",
    Base.metadata,
...
class CompanyModel(Base):
    """A CRM company (account) an organization sells to."""
    ...
    __table_args__ = (
        Index("ix_companies_org_id", "organization_id"),
        UniqueConstraint("organization_id", "name", name="uq_companies_org_name"),
    )


class ContactModel(Base):
    """A CRM contact. ``phone`` (E.164) is the join key for calls: every
    completed voice run upserts a contact by phone number, so the contact
    timeline is the CRM projection of call history.

    ``do_not_call`` and the consent fields are compliance requirements for
    outbound dialing, not optional metadata — the campaign dispatcher
    refuses to dial a DNC contact.
    """
    ...
    phone = Column(String(32), nullable=True, index=True)  # E.164
    company_id = Column(
        Integer, ForeignKey("companies.id", ondelete="SET NULL"), nullable=True
    )
    source = Column(
        Enum(*[s.value for s in ContactSource], name="contact_source"),
        nullable=False,
        default=ContactSource.MANUAL.value,
        server_default=text("'manual'::contact_source"),
    )
    lifecycle_stage = Column(
        Enum(
            *[s.value for s in ContactLifecycleStage],
            name="contact_lifecycle_stage",
        ),
        nullable=False,
        default=ContactLifecycleStage.LEAD.value,
        server_default=text("'lead'::contact_lifecycle_stage"),
    )
    owner_id = Column(Integer, ForeignKey("users.id"), nullable=True)
    ...
    owner = relationship("UserModel")
    tags = relationship("TagModel", secondary=contact_tags_association, lazy="selectin")

    __table_args__ = (
        Index("ix_contacts_org_id", "organization_id"),
        Index("ix_contacts_org_phone", "organization_id", "phone"),
        Index(
            "uq_contacts_org_phone",
            "organization_id",
            "phone",
            unique=True,
...
class TagModel(Base):
    """Org-scoped CRM tag for contact segmentation and campaign targeting."""
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "name", name="uq_crm_tags_org_name"),
    )


class DealModel(Base):
    """A CRM deal attached to a contact and/or company."""
    ...
    value = Column(Float, nullable=True)  # major currency units
    currency = Column(String(3), nullable=False, default="USD")
    pipeline = Column(String(64), nullable=False, default="default")
    stage = Column(String(64), nullable=False, default="lead")
    probability = Column(Integer, nullable=True)  # 0-100
    expected_close_date = Column(DateTime(timezone=True), nullable=True)
    owner_id = Column(Integer, ForeignKey("users.id"), nullable=True)
    status = Column(
        Enum(*[s.value for s in DealStatus], name="deal_status"),
        nullable=False,
        default=DealStatus.OPEN.value,
        server_default=text("'open'::deal_status"),
    )
    ...
    __table_args__ = (
        Index("ix_deals_org_id", "organization_id"),
        Index("ix_deals_org_status_stage", "organization_id", "status", "stage"),
        Index("ix_deals_contact_id", "contact_id"),
    )


class ActivityModel(Base):
    """A CRM timeline entry. Voice calls auto-create ``call`` activities from
    workflow completion (``workflow_run_id`` links the run), which is what
    turns the voice platform into a CRM: every call shows up on the contact
    timeline with recording, transcript and disposition."""
    ...
    deal_id = Column(
        Integer, ForeignKey("deals.id", ondelete="SET NULL"), nullable=True
    )
    type = Column(
        Enum(*[t.value for t in ActivityType], name="crm_activity_type"),
        nullable=False,
    )
    ...
    created_by = Column(Integer, ForeignKey("users.id"), nullable=True)
    due_at = Column(DateTime(timezone=True), nullable=True)  # for tasks
    ...
    __table_args__ = (
        Index("ix_crm_activities_org_id", "organization_id"),
        Index("ix_crm_activities_contact_created", "contact_id", "created_at"),
        Index(
            "ix_crm_activities_open_tasks",
            "organization_id",
            "due_at",
            postgresql_where=text("type = 'task' AND completed_at IS NULL"),
        ),
        Index(
            "uq_crm_activities_run_call",
            "workflow_run_id",
            unique=True,
            postgresql_where=text("type = 'call' AND workflow_run_id IS NOT NULL"),
        ),
    )


class SubscriptionModel(Base):
    """Stripe-backed commercial subscription for an organization.

    One row per organization (enforced unique). Stripe is the source of truth
    for plan/status/period; this table caches it so quota checks don't call
    Stripe on the hot path.
    """
    ...
    plan = Column(
        Enum(*[p.value for p in PlanTier], name="plan_tier"),
        nullable=False,
        default=PlanTier.FREE.value,
        server_default=text("'free'::plan_tier"),
    )
    status = Column(String(32), nullable=False, default="inactive")
    seats = Column(Integer, nullable=False, default=1, server_default=text("1"))
