...
class ARIConnection:
    ...
    def __init__(
        ...
        # Asterisk; Auravox generates the latter so no two configurations can
    ...
    def _should_deactivate(self, failure: AuravoxFailure) -> bool:
    ...
    async def _record_failure(self, failure: AuravoxFailure, **context) -> None:
    ...
    async def _deactivate(self, failure: AuravoxFailure) -> None:
    ...
    async def _create_external_media(
        ...
        # e.g. wss://api.auravox.ai/ws/ari?workflow_id=1&organization_id=2&workflow_run_id=3
        vparams = (
    ...
    async def _handle_stasis_start(
        ...
        """
        ext_channel_id = f"auravox-ext-{uuid.uuid4()}"
        try:
            logger.info(
...
class ARIManager:
    ...
    async def _deactivate_invalid_config(self, row, failure: AuravoxFailure) -> None:
