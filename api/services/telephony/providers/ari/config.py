...
class VicidialAgentAPIConfiguration(BaseModel):
    ...
    source: str = Field(default="auravox", description="VICIdial API source tag")
...
class VicidialNonAgentAPIConfiguration(BaseModel):
    ...
    source: str = Field(default="auravox", description="Non-agent API source tag")
...
class ARIConfigurationRequest(BaseModel):
    ...
    ws_client_name: str = Field(
        default="",
        description="websocket_client.conf connection name for externalMedia (e.g., auravox_staging)",
    )
