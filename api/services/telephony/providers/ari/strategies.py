...
class ARIHangupStrategy(HangupStrategy):
    ...
    async def _terminate_external_pbx_if_any(self, channel_id: str) -> None:
        ...
        never blocks auravox's own hangup if the upstream call fails.
        """
        redis = None
        try:
            import redis.asyncio as aioredis
            ...
            # customer); just let auravox's own legs tear down below.
            ...
            if identity and not transferred and self._external_pbx_adapter:
                ...
                logger.info(
                    ...
                    f"terminating customer leg via API before dropping auravox leg"
                )
