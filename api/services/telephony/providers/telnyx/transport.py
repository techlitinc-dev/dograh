...
async def create_transport(
    ...
    *,
    ...
    # not Auravox's: ``inbound_encoding`` is what we *send into the call*
    # (Auravox → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → Auravox).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # Auravox → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → Auravox; from media_format.encoding
