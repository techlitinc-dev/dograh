...
class CampaignCallDispatcher:
    ...
    async def dispatch_call(
        ...
        try:
            ...
            # Enforce Do-Not-Call (DNC) compliance before dialing
            if await db_client.contact_is_do_not_call(campaign.organization_id, phone_number):
                logger.info(
                    f"Skipping call for queued run {queued_run.id}: phone {phone_number} is on Do-Not-Call list"
                )
                await db_client.update_queued_run(
                    queued_run_id=queued_run.id,
                    state="skipped",
