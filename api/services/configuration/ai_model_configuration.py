...
def merge_ai_model_configuration_v2_secrets(
    ...
    if incoming_dict.get("mode") == "auravox" and existing_dict.get("mode") == "auravox":
        incoming_auravox = incoming_dict.get("auravox") or {}
        existing_auravox = existing_dict.get("auravox") or {}
        incoming_key = incoming_auravox.get("api_key")
        existing_key = existing_auravox.get("api_key")
        if incoming_key and existing_key and contains_masked_key(incoming_key):
            incoming_auravox["api_key"] = resolve_masked_api_keys(
                incoming_key,
                existing_key,
            )
...
def convert_legacy_ai_model_configuration_to_v2(
    ...
    auravox_key = _first_auravox_api_key(configuration)
    if auravox_key:
        return _convert_any_auravox_legacy_configuration(configuration, auravox_key)
...
def auravox_embeddings_base_url() -> str:
...
def apply_managed_embeddings_base_url(
    *,
    ...
    if provider == ServiceProviders.AURAVOX.value or provider == ServiceProviders.AURAVOX:
        return auravox_embeddings_base_url()
    return base_url
...
def _convert_any_auravox_legacy_configuration(
    configuration: EffectiveAIModelConfiguration,
    auravox_key: str,
) -> OrganizationAIModelConfigurationV2:
    ...
    try:
        ...
        speed = 1.0
    if not AURAVOX_SPEED_MIN <= speed <= AURAVOX_SPEED_MAX:
        speed = 1.0
    return OrganizationAIModelConfigurationV2(
        mode="auravox",
        auravox=AuravoxManagedAIModelConfiguration(
            api_key=auravox_key,
            voice=getattr(configuration.tts, "voice", AURAVOX_DEFAULT_VOICE)
            or AURAVOX_DEFAULT_VOICE,
            speed=speed,
            language=getattr(configuration.stt, "language", AURAVOX_DEFAULT_LANGUAGE)
            or AURAVOX_DEFAULT_LANGUAGE,
        ),
    )


def _first_auravox_api_key(configuration: EffectiveAIModelConfiguration) -> str | None:
    for service in (
        configuration.llm,
        configuration.tts,
        configuration.stt,
        ...
        if service is None or _provider(service) != ServiceProviders.AURAVOX:
            continue
        try:
