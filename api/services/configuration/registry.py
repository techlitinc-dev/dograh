...
AURAVOX_PROVIDER_MODEL_CONFIG = provider_model_config("Auravox")
...
AURAVOX_LLM_MODELS = ["default", "accurate", "fast", "lite", "zen"]
AWS_BEDROCK_MODELS = [
...
@register_llm
class AuravoxLLMService(BaseLLMConfiguration):
    model_config = AURAVOX_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AURAVOX] = ServiceProviders.AURAVOX
    model: str = Field(
        default="default",
        description="Auravox-hosted model tier.",
        json_schema_extra={"examples": AURAVOX_LLM_MODELS, "allow_custom_input": True},
    )
...
class GoogleTTSConfiguration(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="chirp_3_hd",
        description=(
            "Google Cloud low-latency TTS engine. Auravox maps this to Pipecat's "
...
AURAVOX_TTS_MODELS = ["default"]


@register_tts
class AuravoxTTSService(BaseTTSConfiguration):
    model_config = AURAVOX_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AURAVOX] = ServiceProviders.AURAVOX
    model: str = Field(
        default="default",
        description="Auravox TTS tier.",
        json_schema_extra={"examples": AURAVOX_TTS_MODELS},
    )
    voice: str = Field(
        default="default",
...
# Auravox STT Service
AURAVOX_STT_MODELS = ["default"]
AURAVOX_STT_LANGUAGES = DEEPGRAM_LANGUAGES
# Languages auto-detected when the Auravox STT language is "multi". Auravox STT runs
# Deepgram Flux multilingual under the hood, which only auto-detects this subset —
# not the full AURAVOX_STT_LANGUAGES list offered for explicit single-language selection.
AURAVOX_MULTILINGUAL_AUTODETECT_LANGUAGES = DEEPGRAM_FLUX_MULTILINGUAL_LANGUAGES


@register_stt
class AuravoxSTTService(BaseSTTConfiguration):
    model_config = AURAVOX_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AURAVOX] = ServiceProviders.AURAVOX
    model: str = Field(
        default="default",
        description="Auravox STT tier.",
        json_schema_extra={"examples": AURAVOX_STT_MODELS},
    )
    language: str = Field(
        default="multi",
        description="Language code; use 'multi' for auto-detect.",
        json_schema_extra={"examples": AURAVOX_STT_LANGUAGES},
    )
...
AURAVOX_EMBEDDING_MODELS = ["auravox_embedding_v1"]


@register_embeddings
class AuravoxEmbeddingsConfiguration(BaseEmbeddingsConfiguration):
    model_config = AURAVOX_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AURAVOX] = ServiceProviders.AURAVOX
    model: str = Field(
        default="auravox_embedding_v1",
        description="Auravox-managed embedding model.",
        json_schema_extra={"examples": AURAVOX_EMBEDDING_MODELS},
    )
