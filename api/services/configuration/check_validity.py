...
class UserConfigurationValidator:
    def __init__(self):
        self._auravox_service_key_validation_cache: dict[str, bool] = {}
        self._validator_map = {
            ...
            ServiceProviders.AURAVOX.value: self._check_auravox_api_key,
    ...
    async def validate(
        ...
        self._auravox_service_key_validation_cache.clear()
    ...
    def _check_auravox_api_key(self, model: str, api_key: str) -> bool:
        if api_key.startswith("dgr"):
            raise ValueError(
                "You provided a Auravox API key (dgr...) instead of a service key. "
                "Please use a service key (mps...)."
            )
        auth = getattr(self, "_auth_context", {})
        if api_key in self._auravox_service_key_validation_cache:
            return self._auravox_service_key_validation_cache[api_key]
        ...
        self._auravox_service_key_validation_cache[api_key] = is_valid
        return is_valid
