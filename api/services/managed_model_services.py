...
def uses_managed_model_services_v2(
    ...
    return any(
        _is_auravox_service(getattr(ai_model_config, section_name, None))
...
def _is_auravox_service(service: Any) -> bool:
...
def get_auravox_service_api_key(
    ai_model_config: EffectiveAIModelConfiguration,
) -> str | None:
    for section_name in ("llm", "tts", "stt", "embeddings"):
        service = getattr(ai_model_config, section_name, None)
        if not _is_auravox_service(service):
            continue
