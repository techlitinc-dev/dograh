...
from api.constants import DEPLOYMENT_MODE, AURAVOX_MPS_SECRET_KEY, MPS_API_URL
...
class MPSServiceKeyClient:
    ...
    def _get_headers(
        ...
        if DEPLOYMENT_MODE != "oss":
            if AURAVOX_MPS_SECRET_KEY:
                headers["X-Secret-Key"] = AURAVOX_MPS_SECRET_KEY
            if organization_id:
    ...
    async def ensure_cloudonix_domain(
        ...
        *,
        ...
        scoped by ``organization_id`` and authenticated with the Auravox/MPS
    ...
    async def get_billing_pricing(self, organization_id: int) -> dict:
        """Return MPS-owned effective platform and Auravox model prices for an org."""
        if DEPLOYMENT_MODE == "oss":
    ...
    async def report_platform_usage(
        ...
        *,
        ...
        """Report hosted Auravox platform usage for a completed workflow run."""
        if DEPLOYMENT_MODE == "oss":
    ...
    def validate_service_key(
        ...
        """
        Synchronously validate a Auravox service key by checking usage via MPS.
        ...
        so callers never misreport a Auravox outage as a customer configuration error.
        """
