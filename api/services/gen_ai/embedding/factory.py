"""Factory for embedding services, including the Auravox-managed (MPS) path.

Centralizes the provider branching (Azure BYOK / Auravox-managed / OpenAI-compatible
    ...
    *,
    ...
    Args:
        correlation_id: A correlation id already available in context (e.g. the
            running workflow's MPS correlation id). Used for the Auravox provider.
        resolve_correlation: When True and no ``correlation_id`` is supplied, resolve
            one for the Auravox provider via ``resolve_embedding_correlation_id``
    ...
    if provider == ServiceProviders.AURAVOX.value:
        ...
        return AuravoxEmbeddingService(
            db_client=db_client,
            api_key=api_key,
            model_id=model_id,
            base_url=base_url,
