"""Quota checking service for Auravox credits.
...
from api.services.managed_model_services import (
    ...
    get_auravox_service_api_key,
    uses_managed_model_services_v2,
)
from api.services.mps_service_key_client import mps_service_key_client

MINIMUM_AURAVOX_CREDITS_FOR_CALL = 0.10
...
OSS_HOSTED_KEY_QUOTA_EXCEEDED_MESSAGE = (
    "The organization linked to this Auravox service key has insufficient credits. "
    "Please add credits at app.auravox.ai or change providers in Models configurations."
)

SERVICE_TOKEN_ORG_MISMATCH_MESSAGE = (
    "The Auravox service token being used is created from another account. "
    ...
    *,
    ...
    log_failure(
        AuravoxFailure(
            ...
            external_message="Auravox could not verify managed model access.",
            provider="auravox",
            error_owner="operator",
            retryable=None,
        ),
...
def _log_insufficient_auravox_credits(
    *,
    organization_id: int | None = None,
    workflow_run_id: int | None = None,
) -> None:
    log_failure(
        AuravoxFailure(
            ...
            code="auravox-insufficient-credits",
            internal_message="Insufficient Auravox credits",
            external_message="Your organization has insufficient Auravox credits.",
            provider="auravox",
            error_owner="user",
            retryable=False,
        ),
...
def _managed_v2_authorization_failed_result() -> QuotaCheckResult:
    return QuotaCheckResult(
        ...
        error_message="Could not verify Auravox credits. Please try again.",
    )
...
def _service_uses_auravox(service: Any) -> bool:
    provider = getattr(service, "provider", None)
    return (
        provider == ServiceProviders.AURAVOX or provider == ServiceProviders.AURAVOX.value
    )


def _auravox_api_keys(user_config: Any) -> set[str]:
    api_keys: set[str] = set()
    for section_name in ("llm", "stt", "tts", "embeddings"):
        service = getattr(user_config, section_name, None)
        if not _service_uses_auravox(service):
            continue
...
async def _authorize_hosted_workflow_run_start(
    *,
    ...
    service_key = (
        get_auravox_service_api_key(user_config) if requires_correlation else None
    )
    if requires_correlation and not service_key:
        _log_mps_system_failure(
            "invalid-service-key",
            "Managed-v2 workflow configuration has no Auravox service key",
            organization_id=organization_id,
            workflow_run_id=workflow_run_id,
        )
        return QuotaCheckResult(
    ...
    try:
        authorization = await mps_service_key_client.authorize_workflow_run_start(
            ...
            minimum_credits=MINIMUM_AURAVOX_CREDITS_FOR_CALL,
            created_by=(
            ...
            metadata={
                "auravox_user_id": str(workflow_owner.id),
    ...
    if (
        ...
        or remaining < MINIMUM_AURAVOX_CREDITS_FOR_CALL
    ):
        _log_insufficient_auravox_credits(
            organization_id=organization_id,
            workflow_run_id=workflow_run_id,
        )
    ...
    logger.info(
        "Auravox run authorization passed for org {}: {:.2f} credits remaining",
        organization_id,
        remaining,
    )
    return QuotaCheckResult(has_quota=True)
...
async def _authorize_oss_auravox_keys(
    *,
    auravox_api_keys: set[str],
) -> QuotaCheckResult:
    """Check per-key MPS credits for OSS deployments before a run starts."""
    for api_key in auravox_api_keys:
        try:
            ...
            if remaining < MINIMUM_AURAVOX_CREDITS_FOR_CALL:
                _log_insufficient_auravox_credits()
                return _insufficient_oss_quota_result()

            logger.info(
                f"Auravox quota check passed for key ...{api_key[-8:]}: "
...
async def _authorize_oss_managed_v2_correlation(
    *,
    ...
    service_key = get_auravox_service_api_key(user_config)
    if not service_key:
        _log_mps_system_failure(
            "invalid-service-key",
            "OSS managed-v2 workflow configuration has no Auravox service key",
            workflow_run_id=workflow_run_id,
        )
        return QuotaCheckResult(
            has_quota=False,
...
async def _authorize_oss_managed_v2_run(
    *,
    ...
    try:
        ...
        legacy_quota = await _authorize_oss_auravox_keys(
            auravox_api_keys={service_key},
        )
    ...
    if (
        ...
        _log_insufficient_auravox_credits(workflow_run_id=workflow_run_id)
    ...
    logger.info(
        "Auravox run authorization passed for key ...{}: {:.2f} credits remaining",
        service_key[-8:],
        remaining,
    )
    return QuotaCheckResult(has_quota=True)
...
async def authorize_workflow_run_start(
    *,
    ...
    try:
        ...
        auravox_api_keys = _auravox_api_keys(user_config)
        if workflow_run_id is None or not uses_managed_model_services_v2(user_config):
            if auravox_api_keys:
                return await _authorize_oss_auravox_keys(
                    auravox_api_keys=auravox_api_keys,
                )
            return QuotaCheckResult(has_quota=True)

        correlation_service_key = get_auravox_service_api_key(user_config)
        ...
        keys_requiring_legacy_check = auravox_api_keys - {correlation_service_key}
        if keys_requiring_legacy_check:
            oss_result = await _authorize_oss_auravox_keys(
                auravox_api_keys=keys_requiring_legacy_check,
            )
