...
def make_storage_audio_sink(
    ...
    """
    Build the observer ``audio_sink``: writes each WAV segment to Auravox's
...
def build_deferred_observer(
    *,
    ...
    return NoveumTraceObserver(
        ...
        # Auravox's on_call_finished calls _finish_conversation itself, AFTER
        # auravox's own on_pipeline_finished handler has run stop_recording()
...
def build_payload_envelope(
    ...
    persisted inside ``workflow_run.logs`` alongside Auravox's own keys, and one
    non-JSON-serializable span attribute would otherwise make the whole logs
    commit raise — silently discarding Auravox's data too. The recursive
