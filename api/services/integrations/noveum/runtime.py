...
def _format_model_label(provider: Any, model: Any) -> str:
    # provider/model may be enum members (e.g. ServiceProviders.AURAVOX) —
...
class NoveumRuntimeSession(IntegrationRuntimeSession):
    ...
    def attach(self, task: Any) -> None:
        ...
        # ABP recording (that is async); auravox's on_client_connected already
    ...
    async def on_call_finished(
        ...
        *,
        ...
        # teardown cannot run before auravox's stop_recording() has flushed the
        ...
        if disposition:
            attributes["auravox.call_disposition"] = disposition
...
def create_runtime_sessions(
    ...
    call_attributes: dict[str, Any] = {
        "auravox.workflow_run_id": context.workflow_run_id,
        "auravox.mode": getattr(context.workflow_run, "mode", None),
        "auravox.agent_version": getattr(context.run_definition, "version_number", None),
