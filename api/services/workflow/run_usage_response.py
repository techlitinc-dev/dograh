...
def format_public_cost_info(
    ...
    auravox_token_usage = 0
    if cost_info:
        if "auravox_token_usage" in cost_info:
            auravox_token_usage = cost_info.get("auravox_token_usage") or 0
        elif "total_cost_usd" in cost_info:
            auravox_token_usage = round(
                float(cost_info.get("total_cost_usd", 0)) * 100, 2
            )

    if duration is None and auravox_token_usage == 0:
        return None

    return {
        "auravox_token_usage": auravox_token_usage,
