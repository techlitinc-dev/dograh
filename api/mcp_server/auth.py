...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated Auravox user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `auravox.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `auravox.org_id` — that attribute triggers the
