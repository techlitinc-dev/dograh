...
export default function ModelConfigurationV2({ docsUrl }: { docsUrl?: string }) {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            if (
                (defaultsResult.error && defaultsResult.response?.status === 403) ||
                (configResult.error && configResult.response?.status === 403)
            ) {
    ...
    if (platformManaged) {
        return <PlatformManagedNotice subject="AI models" />;
    }
